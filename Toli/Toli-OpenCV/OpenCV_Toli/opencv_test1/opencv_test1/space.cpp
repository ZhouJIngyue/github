...
{
	Space_Unit_Vector ans;
	double length;
	length=sqrt(s_vector.x*s_vector.x
		+s_vector.y*s_vector.y+s_vector.z*s_vector.z);
	ans.x=s_vector.x/length;
	ans.y=s_vector.y/length;
	ans.z=s_vector.z/length;
	return ans;
}
...
{
	Space_Point ans;
	ans.x=point_start.x+dir.x*length;
	ans.y=point_start.y+dir.y*length;
	ans.z=point_start.z+dir.z*length;
	return ans;
}